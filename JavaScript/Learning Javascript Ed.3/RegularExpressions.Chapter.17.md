...
It is regularely abbreviated to _regex_.
...
input.endsWith('going', 10);	// True, treat index 14 as end of the string.
...
Regexes are represented by the class _RegExp_:

```js
const re1 = /going/;					// this is how you search for 'going' in regex
const re2 = new RegExp("going");		// equivalent obejct constructor
```
...
```
This can also be done with the regex as a literal:
...
Regex is a pattern for comsuming input strings.    
...
* If there is a match, all the characters are consumed at once. Matching continues with the next character. It does not detect both words if there are 2 overlapped.
...
Alteration is signaled by the `|` character. The `ig` signifies to _ignore case_ (i), and _search globally_ (g).
...
Using our previus beer example, if we wanted to match numbers of more digits, we might do something like:
...
There is a better way to search for multiple character numebers:
...
* ?			--		{0, 1}							/[a-z]\d?/i
* *			--		zero or more (_Klene star_)		/[a-z]\d*/i
* + 		--		one or more						/[a-z]\d+/i
```
...
Period `.` means _match anuthing_ except new lines. It is very often used to consume things that are unimportant. 
...
If you were matching domain names, but only with _.com_ and _.org_ and _.edu_:
...
Regular expressions, by default, are _greedy_. Meaning they will match as much as possible before stopping.

Consider you have some HTML you want to replace, for example `<i>` into `<strong>`:
...
Because it is greedy, after it sees the `<i>`, it will not stop until it discovers the `</i>`.    
...
The only difference is the question mark `?` indide the `(.*?)`. Now it will stop as soon as it sees a `<i>`.     
...
This is kinda useless, it seems.
...
The first froup will match either single or double quotes, followed by a zero or more characters (note this is lazy), followed by `\1` refering to whatever the first match was, either single or double quote. 
...
Following the prev. example, changing all `<a>`'s into something else and keep `class`, `id`, and `href`. But this time, the input is messy.
...
We can deal with the big number of tag variation by using two regexes: one to recognize `<a>` tags, another to replace the contants of the tag with only what we want.

Considering the second part, we can not guarantee that the attributes are in order. We can solve this by using the _String.prototype.split_ method to consider one attrubute at a time:
...
The word boundary metacharacters are `\b` and the inverse, `\B`. They don not consume input.    
...
They don't consume input and you can match any subexpression, they are general purpose.     
